...
arkhe.scheduler
--------------------
Scheduler – Sistema de agendamento de tarefas para o Arkhe.
...
class Job:
    ...
    def persistent(
        ...
        path:
            Pasta de persistência (default: ``.arkhe/scheduler``).
        """
        if isinstance(strategy, str):
            strategy = MissedStrategy(strategy)
        self._missed_strategy = strategy

        base = Path(path) if path else Path(".arkhe") / "scheduler"
...
class Scheduler:
    """
    Fachada estática para o sistema de agendamento do Arkhe.
