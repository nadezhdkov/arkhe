...
arkhe.cli
----------
CLI entry point for the Arkhe ecosystem.
Registered via pyproject.toml: arkhe = "arkhe.cli:main"
"""
...
app = typer.Typer(
    name="arkhe",
    help="Arkhe — Modern utility and game framework for Python",
...
def init(
    ...
    """Initialize a new Arkhe project in the current directory."""

    # Resolve project name
    if name is None:
        name = Path.cwd().name

    typer.echo(f"\n🪺  Initializing Arkhe project: [bold]{name}[/bold]\n", color=True)
...
def doctor() -> None:
    ...
    if Path("project.yml").exists():
        ...
        typer.echo("  –  project.yml not found  (run: arkhe init)")
