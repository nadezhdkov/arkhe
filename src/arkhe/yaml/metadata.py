...
arkhe.yaml.metadata
--------------------
Manages persistent registry metadata and file hashes/mtimes in .arkhe/
"""
...
class MetadataManager:
    """
    Manages `.arkhe/` storage for incremental scanning and registry persistence.
    """

    def __init__(self, project_root: Path) -> None:
        self.project_root = project_root.resolve()
        self.arkhe_dir = self.project_root / ".arkhe"
        self.index_file = self.arkhe_dir / "yaml_index.json"
        self.meta_file = self.arkhe_dir / "yaml_metadata.json"
    ...
    def _ensure_dir(self) -> None:
        if not self.arkhe_dir.exists():
            self.arkhe_dir.mkdir(parents=True, exist_ok=True)
