...
from arkhe.pyunix.animation import Animator
...
def _physics_world():
    from arkhe.pyunix.physics import PhysicsWorld
    return PhysicsWorld
...
class SpriteSystem:
    ...
    def draw(func: Callable) -> Callable:
        """Called every frame for custom rendering. Use Canvas/Draw API internally."""
...
class Entity:
    ...
    def add_component(self, component: Any) -> Any:
        """
        Attach a Unity-style Component to this entity.
    ...
    def has_component(self, comp_type: Type[T]) -> bool:
        return comp_type in self._components

    def remove_component(self, comp_type: Type[T]) -> None:
        comp = self._components.pop(comp_type, None)
        if comp and hasattr(comp, "on_detach"):
            comp.on_detach()
    ...
    def draw_self(self, offset: Tuple[float, float] = (0.0, 0.0)) -> None:
        """
        Blit `self.image` onto the Canvas with transform (rotation, scale, alpha, tint, offset).
        ...
        """
        surface = Canvas._screen
        if not _HAS_PYGAME or not self.visible or self.image is None or not surface:
            return
    ...
    def _dispatch(self, hook: str, *args: Any, **kwargs: Any) -> None:
        ...
        # 1. Fire entity local hooks (decorators)
        for func in self._hooks[hook]:
            func(*args, **kwargs)
            
        # 2. Propagate to components
        comp_method_name = f"on_{hook}"
        for comp in list(self._components.values()):
            method = getattr(comp, comp_method_name, None)
            if method:
                method(*args, **kwargs)
