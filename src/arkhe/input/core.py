"""
arkhe.input.core — InputBuilder and the top-level ask() factory.
...
from arkhe.input.validators import ValidatorFn
...
class InputBuilder:
    ...
    def _read_with_timeout(self, prompt_str: str) -> str:
        if sys.platform == "win32":
            ...
            import warnings
            warnings.warn(
                "arkhe.input: timeout() is not supported on Windows; "
...
def ask(prompt: str) -> InputBuilder:
    ...
    This is the single entry-point for the entire ``arkhe.input`` package.
