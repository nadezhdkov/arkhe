...
arkhe.trying
----------------
Try – Tratamento funcional de erros para o Arkhe.
...
class Try(Generic[T]):
    ...
    * ``Failure(error)`` — operação falhou.
    ...
    def to_promise(self) -> Any:
        """
        Converte para Promise (requer arkhe.promise).
        ...
        try:
            ...
            try:
                ...
                raise ImportError(
                    ...
                    "Instale arkhe-promise ou coloque promise.py no mesmo diretório."
                )
        if self.is_success():
